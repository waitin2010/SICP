...
(define (substitute ls old new)
  (define (replace ls)
    (cond ((list? ls) (map replace ls))
	  ((eq? ls old) new)
	  (else ls)))
  (map replace ls)
 )
(substitute '((lead guitar) (bass guitar) (rhytm guitar)) 'guitar 'axe) 
(substitute '(((((hello))))) 'hello 'goodbye)
;; Question 2
(define (substitute2 ls old-ls new-ls)
  (cond ((empty? old-ls) ls)
	(else (substitute2 (substitute ls (car old-ls) (car new-ls))
			   (cdr old-ls)
			   (cdr new-ls))))
 )
...
(define (dfs-find tree pred)
  (cond ((pred (datum tree)) (datum tree))
	(else (for-each dfs-find (children tree))))
 )
(define tree '(book ((structure (interpretation computer)) not english)))
(define (begins-with-vowel?  world)
  (member? (first word) '(a e i o u)))
(dfs-find tree begins-with-vowel?)
;; Question 3
